...
// Observability stack + servizi non referenziati dall'api (redis, elasticsearch, kibana):
// pesanti e ridondanti nel dev loop, dove la dashboard Aspire riceve già logs/trace/metriche
// via OTLP. Sempre inclusi in publish (parità col compose generato); in run solo su richiesta:
// Observability:Enabled=true (env var Observability__Enabled=true o appsettings dell'AppHost).
var enableObservability =
    builder.ExecutionContext.IsPublishMode || builder.Configuration.GetValue("Observability:Enabled", false);
